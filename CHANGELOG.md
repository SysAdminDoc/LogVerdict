...
- Added a versioned module ZIP beside the executables; its manifest FileList is copied exactly, the archive is hash- and SBOM-covered, and the README documents process-scoped execution policy and non-administrator capability boundaries.
- Added Constrained Language Mode detection and capability reporting, a fail-closed GUI refusal with named exit code 5, and a `Compress-Archive` evidence-creation fallback while documenting the remaining FullLanguage surfaces.
- Added a pinned, content-free Windows 2k log-parsing benchmark with on-demand corpus fetching, MIT annotation metadata, and regression budgets for the real template masker.
...
- Added a CycloneDX 1.7 SBOM beside each SPDX document and marked the accompanying SLSA-shaped provenance as self-asserted and unsigned; offline verification now checks both SBOM formats and the published SHA-256 values.
...
- Normalized provider timestamps with invariant round-trip parsing into the shared local time basis and evaluated localized rules against the source provider locale before falling back to the reviewer culture.
- Retained the newest text-log matches with explicit truncated coverage and applied byte, record, and elapsed collection budgets while bounding WER crash-artifact inventory.
- Added a license-provenance-checked provider message-template cache importer, live/offline template recovery, and evidence-bundle transport for signatures whose local message resources are absent.
- Reconciled the verdict schema with the correlation engine, documented sequential never-reused rule ids and deprecated tombstones, and made the release gate validate the shipped verdict, fixture, and error-catalog databases plus malformed rejection variants.
- Regenerated the packaged GUI reference with a neutral machine label, refreshed its pinned SHA-256 metadata, and tightened the release hygiene checks for BOM-free JSON and generated supply-chain output.
- Unified coverage records around one validated status vocabulary, mapped parsed offline EVTX manifests to readable or empty coverage, and retained a named truncated row for every event channel left unread after a shared budget stop.
- Rebalanced the Windows rule database toward ordinary breakage with fixture-backed coverage for GPU TDR, Print Spooler, USB enumeration, Windows Audio, Bluetooth, wake sources, DHCP lease renewal, NVMe I/O retries, Task Scheduler starts, and Resource-Exhaustion 2004; Reliability Monitor unknowns are no longer demoted and healthy Sysmon timestamp/alternate-stream noise is re-levelled and documented.
- Adopted the rule lifecycle metadata: the default freshness window is 180 days, volatile/build-specific rulings carry explicit bounds, every rule records its content-change date, supersession links are typed and validated, and the pre-fix Windows Firewall ruling expires when KB5062660 is present.
