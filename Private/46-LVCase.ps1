...
function ConvertTo-LVCaseRecipeQuotedValue {
    [CmdletBinding()]
    param([AllowNull()]$Value)
...
function Get-LVCaseProfileProblems {
    ...
    if ([string]$Profile.name -match '[\r\n:]') { $problems.Add('name may not contain carriage returns, line feeds, or colons') | Out-Null }
    if ([string]$Profile.purpose -match '[\r\n:]') { $problems.Add('purpose may not contain carriage returns, line feeds, or colons') | Out-Null }
    foreach ($note in @($Profile.notes | Where-Object { $null -ne $_ })) {
        if ([string]$note -match '[\r\n:]') {
            $problems.Add('notes may not contain carriage returns, line feeds, or colons') | Out-Null
            break
        }
    }
    foreach ($source in @($Profile.sources | Where-Object { $_ })) {
        if ([string]$source.name -match '[\r\n]') {
            $problems.Add('source names may not contain carriage returns or line feeds') | Out-Null
            break
        }
    }
...
function New-LVCaseProfileObject {
    ...
    if ($Redact) {
        ...
        $purposeText = (ConvertTo-LVRedactedText -Text $purposeText -MachineName $machine -UserName $env:USERNAME) -replace ':', '-'
        $notes = @($notes | ForEach-Object {
            (ConvertTo-LVRedactedText -Text $_ -MachineName $machine -UserName $env:USERNAME) -replace ':', '-'
        })
    }
...
function Get-LVCaseKapeRecipe {
    ...
    $profileId = ConvertTo-LVCaseRecipeQuotedValue $Profile.profileId
    $version = ConvertTo-LVCaseRecipeQuotedValue $Profile.attribution.version
    $lines.Add(('Description: LogVerdict collection profile {0}' -f $profileId)) | Out-Null
    ...
    $lines.Add(('Comment: Generated by LogVerdict {0}; verify source scope and destination before collection.' -f $version)) | Out-Null
    ...
    if ($textSources.Count -gt 0) {
        ...
        $quotedSources = @($textSources | ForEach-Object { ConvertTo-LVCaseRecipeQuotedValue $_ }) -join ', '
        $lines.Add('Comment: Text-log names observed by the profile: ' + $quotedSources) | Out-Null
    }
...
function Get-LVCaseVelociraptorRecipe {
    ...
    $quotedName = ConvertTo-LVCaseRecipeQuotedValue $name
    $quotedProfileId = ConvertTo-LVCaseRecipeQuotedValue $Profile.profileId
    $quotedDaysBack = ConvertTo-LVCaseRecipeQuotedValue $Profile.bounds.daysBack
...
name: $quotedName
description: |
  Deterministic LogVerdict collection recipe for profile $quotedProfileId.
...
parameters:
  - name: DaysBack
    ...
    default: $quotedDaysBack
sources:
  - name: EvidenceFiles
    query: |
      LET profile_id = $quotedProfileId
