# Live scan orchestration. Collection, reduction, resolution, and result projection are
# separate stages so each stage can be exercised without a 600-line public command.

function Invoke-LVScanBaseCollection {
    [CmdletBinding()]
    param(
        [int]$DaysBack,
        [string[]]$Channel,
    ...
    return [pscustomobject][ordered]@{
        ...
        ChannelEnumerationErrorCount = [int]$channelEnumerationErrorCount
        ChannelEnumerationStatus = [string]$channelEnumerationStatus
        Records = $records
        SequenceNotes = @($sequenceNotes)
        Crash = @($crash)
        SetupDiag = $setupDiag
        SetupDiagStatus = $setupDiagStatus
        SetupDiagBudgetStop = $setupDiagBudgetStop
        CrashBudgetStop = $crashBudgetStop
        DecodedCrashCount = [int]$decodedCrashCount
...
function Invoke-LVScanProviderCollection {
    [CmdletBinding()]
    param(
    ...
    $collectionBudget = $Context.CollectionBudget
    ...
    $providerExtensions = $Context.ProviderExtensions
    $providerCoverage = $Context.ProviderCoverage
    $providerProjections = $Context.ProviderProjections
    if ($ProviderPath) {
    ...
    $Context.ProviderExtensions = $providerExtensions
    $Context.ProviderCoverage = $providerCoverage
    $Context.ProviderProjections = $providerProjections
    return $Context
}

function Complete-LVScanReduction {
    [CmdletBinding()]
    ...
    $Context.Reduction = $stat
    return $Context
}

function Invoke-LVScanResolution {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]$Context,
        [string]$DatabasePath,
    ...
    $Context.IncidentReduction = $incidentReduction
    $Context.Incidents = @($incidents)
    $Context.LowConfidenceSuppressed = [int]$lowConfidenceSuppressed
    $Context.SuppressionSummary = $suppressionSummary
    $Context.Database = $db
    $Context.DatabaseFreshness = $databaseFreshness
    $Context.InstalledKbs = @($installedKbs)
    $Context.ModelRequested = $modelRequested
    $Context.PromotedDrafts = @($promotedDrafts)
    $Context.CurrentWindowsBuild = $currentWindowsBuild
    return $Context
}

function Complete-LVScanCoverage {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]$Context,
        [int]$DaysBack,
    ...
    $channelEnumerationFailed = $Context.ChannelEnumerationFailed
    $channelEnumerationFailures = $Context.ChannelEnumerationFailures
    $channelEnumerationErrorCount = $Context.ChannelEnumerationErrorCount
    $crash = $Context.Crash
    $crashBudgetStop = $Context.CrashBudgetStop
    $decodedCrashCount = $Context.DecodedCrashCount
    $diagnosticEvidence = $Context.DiagnosticEvidence
    $providerCoverage = $Context.ProviderCoverage
    $records = $Context.Records
    $sequenceNotes = $Context.SequenceNotes
    $setupDiagStatus = $Context.SetupDiagStatus
    $setupDiag = $Context.SetupDiag
    $reliabilityRecords = $Context.ReliabilityRecords
    ...
    $Context.HorizonWarning = $horizonWarning
    $Context.CoverageNotes = $coverageNotes
    $Context.CoverageSources = $coverageSources
    $Context.HealthProfiles = @($healthProfiles)
    return $Context
}

function ConvertTo-LVScanResult {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]$Context,
        [AllowNull()]$CaseProfile,
        [switch]$Redact,
        [switch]$PerformanceTelemetry,
        [switch]$AllChannels,
        [switch]$DiagnosticChannels,
        ...
        [switch]$ExplainUnknown,
        [switch]$PromoteToRule,
        [long]$MaxCollectionBytes,
        [int]$MaxCollectionRecords,
        [int]$MaxCollectionSeconds
    )

    $started = $Context.Started
    $scanTimer = $Context.ScanTimer
    $performance = $Context.Performance
    $performanceEnabled = $Context.PerformanceEnabled
    $all = $Context.All
    $findings = $Context.Findings
    ...
    $channelEnumerationFailures = $Context.ChannelEnumerationFailures
    $channelEnumerationErrorCount = $Context.ChannelEnumerationErrorCount
    $channelEnumerationStatus = $Context.ChannelEnumerationStatus
    $coverageNotes = $Context.CoverageNotes
    $coverageSources = $Context.CoverageSources
    $healthProfiles = $Context.HealthProfiles
    $providerExtensions = $Context.ProviderExtensions
    $providerProjections = $Context.ProviderProjections
    $stat = $Context.Reduction
    $incidents = $Context.Incidents
    $incidentReduction = $Context.IncidentReduction
    $lowConfidenceSuppressed = $Context.LowConfidenceSuppressed
    $suppressionSummary = $Context.SuppressionSummary
    $crash = $Context.Crash
    $setupDiagStatus = $Context.SetupDiagStatus
    $horizon = $Context.Horizon
    $horizonWarning = $Context.HorizonWarning
    $stability = $Context.Stability
    $elevated = $Context.Elevated
    $channels = $Context.Channels
    $channelMode = $Context.ChannelMode
    $channelStatus = $Context.ChannelStatus
    $currentWindowsBuild = $Context.CurrentWindowsBuild
    $db = $Context.Database
    $databaseFreshness = $Context.DatabaseFreshness
    $installedKbs = $Context.InstalledKbs
    $advisoryContext = $Context.AdvisoryContext
    $modelRequested = $Context.ModelRequested
    $promotedDrafts = $Context.PromotedDrafts
...
function Invoke-LVLiveScan {
    [CmdletBinding()]
    param(
        [int]$DaysBack, [string[]]$Channel, [switch]$AllChannels, [switch]$DiagnosticChannels,
        [switch]$SkipTextLogs, [switch]$SkipReliability, [switch]$PerformanceTelemetry,
        [switch]$IncludeBenign, [switch]$IncludeLowConfidence, [string]$SuppressionPath,
        [string]$DatabasePath, [switch]$Redact, [switch]$ExplainUnknown,
        [string]$OllamaModel, [string]$OllamaEndpoint, [switch]$PromoteToRule,
        [string]$LocalRulePath, [string]$HistoryPath, [int]$HistoryWindowDays,
        [string]$AdvisoryPath, [string]$AdvisoryPackage, [string]$AdvisoryVersion,
        [string[]]$ProviderPath, [string]$ProviderTemplatePath, [switch]$AllowUntrustedProvider,
        [long]$MaxCollectionBytes, [int]$MaxCollectionRecords, [int]$MaxCollectionSeconds,
        [Parameter(Mandatory)]$CollectionBudget, [AllowNull()]$CaseProfile
    )

    $context = Invoke-LVScanBaseCollection -DaysBack $DaysBack -Channel $Channel -AllChannels:$AllChannels `
        -DiagnosticChannels:$DiagnosticChannels -SkipTextLogs:$SkipTextLogs -SkipReliability:$SkipReliability `
        -PerformanceTelemetry:$PerformanceTelemetry -AdvisoryPath $AdvisoryPath -AdvisoryPackage $AdvisoryPackage `
        -AdvisoryVersion $AdvisoryVersion -ProviderTemplatePath $ProviderTemplatePath -CollectionBudget $CollectionBudget
    $context = Invoke-LVScanProviderCollection -Context $context -ProviderPath $ProviderPath `
        -AllowUntrustedProvider:$AllowUntrustedProvider -DaysBack $DaysBack -SkipTextLogs:$SkipTextLogs -SkipReliability:$SkipReliability
    $context = Complete-LVScanReduction -Context $context
    $context = Invoke-LVScanResolution -Context $context -DatabasePath $DatabasePath -SuppressionPath $SuppressionPath `
        -IncludeBenign:$IncludeBenign -IncludeLowConfidence:$IncludeLowConfidence -ExplainUnknown:$ExplainUnknown `
        -OllamaModel $OllamaModel -OllamaEndpoint $OllamaEndpoint -PromoteToRule:$PromoteToRule `
        -LocalRulePath $LocalRulePath -Redact:$Redact
    $context = Complete-LVScanCoverage -Context $context -DaysBack $DaysBack -SkipTextLogs:$SkipTextLogs `
        -SkipReliability:$SkipReliability -HistoryPath $HistoryPath -HistoryWindowDays $HistoryWindowDays
    return ConvertTo-LVScanResult -Context $context -CaseProfile $CaseProfile -Redact:$Redact `
        -PerformanceTelemetry:$PerformanceTelemetry -AllChannels:$AllChannels -DiagnosticChannels:$DiagnosticChannels `
        -SkipTextLogs:$SkipTextLogs -SkipReliability:$SkipReliability -IncludeBenign:$IncludeBenign `
        -IncludeLowConfidence:$IncludeLowConfidence -ExplainUnknown:$ExplainUnknown -PromoteToRule:$PromoteToRule `
