...
Unknown signatures are also checked against the bundled [`Data/error-codes.json`](Data/error-codes.json) reference catalog. The catalog currently carries 2,745 Microsoft WinError.h statuses, 378 Microsoft kernel bug-check codes, and 13 common HRESULTs, with the official name, description, explanation, and source link. A catalog match improves the explanation but stays `unknown` until provider-specific context justifies a reviewed verdict. Regenerate it from current Microsoft Learn tables with `Tools\Import-MicrosoftErrorCatalog.ps1`; the normal scan never makes a network call.
...
- **Coverage is the roadmap.** 180 rules ship, alongside a 3,136-entry Microsoft error and stop-code catalog. A first scan will still report `unknown` signatures - that is the tool refusing to guess, and each one is a candidate rule.
